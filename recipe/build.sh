...
if [[ `uname` == 'Darwin' ]]; then
	...
	sed -i '' 's/^CFLAGS =/CFLAGS = $\{CFLAGS\} -headerpad_max_install_names/' Makefile
	sed -i '' '/^\tSHARED_LIB_FLAG = -dynamiclib/ s/$/ $\{LDFLAGS\}/' Makefile
	cat Makefile
fi
make all
make lib
...
if [[ `uname` == 'Darwin' ]]; then
	install -m644 libsvm.4.dylib $PREFIX/lib/
	ln -s libsvm.4.dylib $PREFIX/lib/libsvm.dylib
else
	install -m644 libsvm.so.4 $PREFIX/lib/
	ln -s libsvm.so.4 $PREFIX/lib/libsvm.so
fi
